...
//   // ----------------- request helpers (use token) -----------------
//   // relative endpoints used earlier in your app
// //  const API = import.meta.env.VITE_API_URL || "http://127.0.0.1:5000";
...
//    if ("main_church" in normalized)
//      normalized.main_church = Number(normalized.main_church || 0);
//    if ("children_ministry" in normalized)
//      normalized.children_ministry = Number(normalized.children_ministry || 0);
...
//       .then((data) => {
//         if (!mounted) return;
//         // ensure dates are strings "YYYY-MM-DD" (backend should already)
...
//       // backend returns the created entry
//       setEntries((prev) => {
...
//     // copy numbers as strings to show in inputs
//     setEditForm({
//       ...entry,
...
//       // remove read-only fields that backend doesn't expect (if any)
...
//           <button onClick={() => window.open(`/finance/export/pdf?start=${startDate}&end=${endDate}`)} className="bg-pink-600 text-white px-6 py-3 rounded-lg">Export PDF</button>
//           <button onClick={() => window.open(`/finance/export/docx?start=${startDate}&end=${endDate}`)} className="bg-pink-600 text-white px-6 py-3 rounded-lg">Export Word</button>
//         </div>
//       </div>
